        ...
        <el-tab-pane label="动态参数" name="many">
          ...
          <el-table :data="manyTableData" border stripe>
            ...
            <el-table-column type="expand">
              <template class="expand" slot-scope="scope">
                <!-- 循环渲染Tag标签 -->
                <el-tag
                  closable
...
export default {
  ...
  methods: {
    ...
    async showEditDialog(attrId) {
      ...
      const { data: res } = await this.$http.get(
        `categories/${this.cataId}/attributes/${attrId}`,
        {
    ...
    showInput(row) {
      ...
      // 让文本框自动获取焦点
      // $nextTick 方法的作用 就是当页面上的元素被重新渲染之后，才会执行回调函数的代码
      this.$nextTick(_ => {
        // 通过$refs拿到这个输入框 然后调用 focus() 函数获取到焦点
